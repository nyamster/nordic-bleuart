...
void ble_uart_ready_data_send(int length)
{
    ...
    while (m_data_cnt < length)
    {
        for (int i = 0; i < 20; i++)
        {
            data[i] = m_data[m_data_cnt+i];
        }
        if ((length - m_data_cnt) < 20)
        {
            err_code = ble_nus_string_send(&m_nus, data, length - m_data_cnt);
            if (err_code == BLE_ERROR_NO_TX_PACKETS ||
            ...
            printf("data sended through uart\n");
            UART_FLAG = false;
        }
        else
        {
...
void ble_uart_data_send()
{
    ...
    while (m_data_cnt < m_data_length+20)
    //while (true)
    {
        ...
        else if (m_data_length <= m_data_cnt && m_data_length % 20 != 0)
        {
            m_data_cnt += 20;
            global_cnt += 10;
            break;
